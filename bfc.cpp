...
{
	if (arr[arrPointer] == 0)
	{				  // don't have to run the loop, just find the closing ] of the loop
		int loop = 1; // if there are nested loops, still don't run them
		while (loop && progPointer < prog.length())
		{ // loop till all the '[' and ']' are matched
			progPointer++;
			if (prog[progPointer] == '[')
				loop++;
			else if (prog[progPointer] == ']')
				loop--;
		}
		progPointer++;
		if (loop)
	...
	{									  // if value at current cell not zero, run the loop
		...
		while (progPointer < prog.length())
		{
			...
			{						 // nested loop
				...
				if (ret == 2)
				{ // if no matching ']', print error
					progPointer = loopStart - 1;
					return 2;
				}
			}
			if (operation() == 1)
				return 1; // if the operation cannot be performed, return error code
			if (prog[progPointer] == ']')
			{
...
int bfc::progRun()
{
	int ret;
	while (progPointer < prog.length())
	{ // Program Pointer runs through the input
		if (prog[progPointer] == '[')
		{						 // Beginning of a loop
			ret = performLoop(); // ret stores whether loop was successful
			if (ret != 0)
			{ // if loop was not successful, print corresponding error
				printError(ret);
				break;
			}
		}
